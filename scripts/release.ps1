...
param(
    ...
    [string]$Version,
    [switch]$NoWait  # Пропустить ожидание CI (как раньше)
)
...
try {
    ...
    # Capture the commit SHA we just pushed — this is the reliable anchor
    $headSha = git rev-parse HEAD

    # Ждём появления run, matching by commit SHA
    while ($elapsed -lt 60) {
    ...
    while ($elapsed -lt $maxWait) {
        ...
        if ($status -eq "completed") {
            if ($conclusion -eq "success") {
                ...
                # Откат: удаляем тег и релиз, ревертим коммит + синхронизируем remote
                ...
                Write-Host ""
                Write-Host "  Rolled back: tag deleted, commit reverted (local + remote)." -ForegroundColor Yellow
