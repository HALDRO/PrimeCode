...
export const Select: React.FC<SelectProps> = ({
	...
	const dropdownItems = useMemo(
		() => options.map(opt => ({ id: opt.value, label: opt.label, data: opt.value })),
		[options],
	);
		...
		<>
			...
			{isOpen && (
				<DropdownMenu
					...
					renderItem={(item, { selected, hovered, onSelect, onHover }) => {
						const isCurrentValue = item.data === value;
						...
						return (
							<div
								...
								className={cn(
									...
									isHighlighted ? 'bg-(--alpha-10) text-vscode-foreground' : 'text-(--alpha-70)',
									isCurrentValue && 'text-(--color-accent) font-medium bg-(--color-accent)/10',
								)}
							>
