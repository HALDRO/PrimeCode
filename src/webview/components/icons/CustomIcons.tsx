...
/**
 * Thinking effort level icon — brain silhouette (same as BrainSideIcon) with dynamic opacity.
 * Opacity scales from dim (default) to bright (max level).
 */
export const ThinkingLevelIcon: React.FC<IconProps & { level?: number; total: number }> = ({
	size = 16,
	style,
	className,
	level,
	...
	// opacity: 0.35 (default/no selection) → 0.5 (lowest) → 1.0 (highest)
	const opacity = level !== undefined && total > 0 ? 0.5 + (level / (total - 1 || 1)) * 0.5 : 0.35;
